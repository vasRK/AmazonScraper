    ...
    try {
        ...
        while (true) {
            ...
            if (completeBookIds.length > 0) {
                ...
                //logger.info(`image extraction completed for book ids ${completeBookIds.join()}`)
            }

            const failedBookIds = extractResponses.filter(res => !res.success).map(res => res.bookId);
            if (failedBookIds.length > 0) {
                logger.error(`image extraction failed for book ids ${failedBookIds.join()}`)
            }

            const msgCompleteReq = messages.map(msg => msg.complete());
            const res = await Promise.all([...msgCompleteReq]);
            logger.info('batch end total messages ', messages.length);
        }
    } catch (err) {
        logger.error('error on while read', err);
    }
    finally {
